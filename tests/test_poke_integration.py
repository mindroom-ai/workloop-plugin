...
class AgentMessageSnapshotStub:
...
def _set_latest_message(
    ...
    *,
    ...
    ctx.latest_messages[(room_id, thread_id, sender)] = AgentMessageSnapshotStub(
        content={module.poke.STREAM_STATUS_KEY: status},
        origin_server_ts=int(
            (datetime.now(UTC) - timedelta(minutes=age_minutes)).timestamp() * 1000
        ),
    )


async def _read_snapshot_from_cache(
    state_root: Path,
    room_id: str,
    sender: str,
    *,
...
async def test_room_scope_reader_uses_real_cache_accessor(tmp_path: Path) -> None:
    ...
    async def get_latest_agent_message_snapshot(
        ...
        *,
        ...
        return await _read_snapshot_from_cache(
            ctx.state_root,
            room_id,
            sender,
            thread_id=thread_id,
